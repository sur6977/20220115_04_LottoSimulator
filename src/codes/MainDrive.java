...
//				내 번호 목록을 돌아보다가 => 입력한 숫자와 같은게 있다면? => 이미 등록된 숫자.(중복) => 사용하면 안된다.
...
//				Math.random()*45 +1 => int로 강제변환(소수점 버림) : 0*45 +1 <= 결과물*45 < 1*45 +1  => 1~45.??? => 1~45
...
//			j가 길이보다 2개 적은값 까지만 돌도록.
			for(int j=0; j<winNumberArr.length-1; j++) {
				
//				이어져있는 2개의 숫자를 비교, 숫자가 잘못되어있다면 바꿔주자.		
...
//		내 번호 하나를 들고 => 당첨번호 비교 6회 반복 => 내 번호 6새에 대해서 통째로 반복 (for 중첩)
//		배열의 내용물을 꺼내보는 for
